...
function deployment() {
  ...
  if [ "$DOCKER" ]; then
    ...
    docker exec -it webserver sh -c "mkdir /etc/nginx/sites-enabled/ && ln -s /etc/nginx/sites-available/patientsv2.clinics.com.tr /etc/nginx/sites-enabled/ && ln -s /etc/nginx/sites-available/inchat.clinics.com.tr /etc/nginx/sites-enabled/"
