<!-- GUIDE Modal Body -->
<div class="modal fade" id="delete_modal_faq-questions" tabindex="-1" role="dialog"
    aria-labelledby="customerDocumentCreateModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content" style="margin-top:30vh;">
            <form action="{{ route('admin.faq-questions.destroy', [0]) }}" method="POST">
...
@parent
    <script>
            $('#delete_modal_faq-questions').on('show.bs.modal', function(event) {
            var button = $(event.relatedTarget);
            var rowId = button.data('id') ?? 0;

            $('#delete_modal_faq-questions').on('click', 'button.submit[type="button"]', function() {
                // get form in this div
                var form =  $('#delete_modal_faq-questions').find('form');
