...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.travel-groups.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="row">
                <div class="col-md-3 offset-2 text-align-center">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" placeholder="Group Name..." name="name" id="name" value="{{ old('name', '') }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('color') ? 'is-invalid' : '' }}" type="text" placeholder="Enter Color..." name="color" id="color" value="{{ old('color', '') }}" required>
                @if($errors->has('color'))
                    <div class="invalid-feedback">
                        {{ $errors->first('color') }}
                    </div>
