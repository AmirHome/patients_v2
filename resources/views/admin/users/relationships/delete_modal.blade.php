...
@parent
    <script>
           $('#delete_modal_users').on('show.bs.modal', function(event) {
        var button = $(event.relatedTarget);
        var rowId = button.data('id') ?? 0;

        $('#delete_modal_users').on('click', 'button.submit[type="button"]', function() {
            var $this = $(this);

            $this.prop('disabled', true);

            var form = $('#delete_modal_users').find('form');
