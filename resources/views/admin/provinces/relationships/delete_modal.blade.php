<!-- GUIDE Modal Body -->
<div class="modal fade" id="delete_modal_provinces" tabindex="-1" role="dialog"
    aria-labelledby="customerDocumentCreateModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content" style="margin-top:30vh;">
            <form action="{{ route('admin.provinces.destroy', [0]) }}" method="POST">
...
@parent
    <script>
           $('#delete_modal_provinces').on('show.bs.modal', function(event) {
        var button = $(event.relatedTarget);
        var rowId = button.data('id') ?? 0;

        $('#delete_modal_provinces').on('click', 'button.submit[type="button"]', function() {
            var $this = $(this);

            $this.prop('disabled', true);

            var form = $('#delete_modal_provinces').find('form');
