...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.campaignOrg.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.campaign-orgs.update", [$campaignOrg->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" placeholder="title" type="text" name="title" id="title" value="{{ old('title', $campaignOrg->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('channel') ? 'is-invalid' : '' }}" name="channel_id" id="channel_id" required>
                    @foreach($channels as $id => $entry)
                        <option value="{{ $id }}" {{ (old('channel_id') ? old('channel_id') : $campaignOrg->channel->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('started_at') ? 'is-invalid' : '' }}" type="text" name="started_at" id="started_at" value="{{ old('started_at', $campaignOrg->started_at) }}" required>
            ...
            <div class="form-group">
                ...
                <div class="row mt-3">
                    ...
                    <div class="mt-2 form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $campaignOrg->status) === (string) $key ? 'checked' : '' }} required>
