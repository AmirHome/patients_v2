    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.travel-statuses.update", [$travelStatus->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" placeholder="Enter Status Name.." type="text" name="title" id="title" value="{{ old('title', $travelStatus->title) }}" required>
                @if($errors->has('title'))
